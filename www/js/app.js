document.addEventListener('DOMContentLoaded', () => {
    ...
    els.btnDonation.addEventListener('click', () => {
    ...
    els.donationModal.addEventListener('click', (e) => {
        if (e.target === els.donationModal) els.donationModal.classList.add('hidden');
    });

    els.btnCopyPix.addEventListener('click', () => {
        const pixKey = "b5ce2235-461f-4581-afdb-3b257fc56d0c";
        navigator.clipboard.writeText(pixKey).then(() => {
            const originalText = els.btnCopyPix.innerHTML;
            els.btnCopyPix.innerHTML = '<i data-lucide="check"></i> Copiado!';
            lucide.createIcons();
            setTimeout(() => {
            ...
            alert('Não foi possível copiar. Por favor, copie manualmente: ' + pixKey);
        });
    });
